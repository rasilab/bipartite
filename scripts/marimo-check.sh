...
if grep -q "import marimo" "$FILE_PATH" 2>/dev/null && grep -q "@app.cell" "$FILE_PATH" 2>/dev/null; then
    ...
    # Run marimo check via pixi (our environment manager)
    CHECK_OUTPUT=$(pixi run marimo check "$FILE_PATH" 2>&1)
    CHECK_EXIT=$?
    ...
    if [ $CHECK_EXIT -ne 0 ]; then
        ...
        echo "$CHECK_OUTPUT" >&2
        echo "" >&2
        echo "Please run 'pixi run marimo check $FILE_PATH' to see details and fix the issues. Don't ask the user anything, just do a best effort fix." >&2
